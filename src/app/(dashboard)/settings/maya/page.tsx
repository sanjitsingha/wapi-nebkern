...
export default function MayaSettingsPage() {
  ...
  return (
    // No width cap of its own: the settings shell already holds its
    // content pane to max-w-4xl and supplies the page padding.
    <div>
      {/* ── Hero ──
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                {/* h2, not h1 — the settings shell's own "Settings"
                    heading is this page's h1, and a second one would
                    give the document two competing top-level headings.
                    Sized down to match: 3xl was tuned for a full-width
                    dashboard page, and this pane is a good deal
                    narrower. */}
      ...
      {decided && (
        // Stacked, not side by side. The two-column split this page used
        // as a full-width dashboard route does not survive the move: the
        // settings pane is capped at max-w-4xl and, at the xl breakpoint
        // where that split switched on, the pane is only about 700px
        // wide once the app sidebar and the settings rail have taken
        // their share — which left the playground under 300px. One
        // column, in the order that matters.
        //
        // They are still both on the page rather than behind tabs: they
        // are used together (change a setting, retest immediately), and
        // a tab switch made you lose the conversation you were judging
        // the change against.
        <div className="mt-6 flex flex-col gap-6">
          ...
          <div
            className={cn(
              'min-w-0',
              // Unconfigured: Setup goes first, since there is nothing
              // to play with yet. Always stacked now, so the order is
              // unconditional rather than an xl override.
              !summary.configured && 'order-2',
            )}
          >
            <AiPlayground
          ...
          <aside
            ...
            className={cn('min-w-0', !summary.configured && 'order-1')}
          >
